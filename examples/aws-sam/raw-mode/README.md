...
```bash
$ aws --profile sw lambda invoke --function-name arn:aws:lambda:us-east-1:790731757232:function:example-wl-raw-sam-app-ExampleRawFunction-AKWQKTCTRPTP --payload fileb://events/event.json /tmp/output.json && cat /tmp/output.json
{
    "StatusCode": 200,
...
```bash
aws-sam-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-sam-app$ sam local start-api
aws-sam-app$ curl http://localhost:3000/
```
...
```bash
aws-sam-app$ sam logs -n HelloWorldFunction --stack-name aws-sam-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name aws-sam-app
```
